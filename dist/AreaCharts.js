...
const AreaChart = ({
  ...
  const [defaultTheme, setDefaultTheme] = useState(theme ? theme : {
    ...
    colors: [am4core.color("#00d084"), am4core.color("#474776"), am4core.color("#65d5c3"), am4core.color("#c7b1a0"), am4core.color("#ab8a88"), am4core.color("#fbc7ac"), am4core.color("#f59891"), am4core.color("#cf899e"), am4core.color("#7e98b5"), am4core.color("#9c9bb1")]
  }),
        getTimeSettings = () => {
    ...
    switch (timeRange) {
      case "currYear":
        timeUnit = "day";
        timeRangeValue = {
          min: momentJalali().startOf(persianMode ? "jYear" : "year").valueOf(),
          max: momentJalali().endOf(persianMode ? "jYear" : "year").valueOf(),
          format: `YYYY MM`,
          jalaliFormat: `jMMMM`
        };
        break;
      ...
      default:
        timeUnit = "hour";
        timeRangeValue = {
          min: momentJalali().startOf(persianMode ? "jMonth" : "month").valueOf(),
          max: momentJalali().endOf(persianMode ? "jMonth" : "month").valueOf(),
          format: `MMM DD`,
          jalaliFormat: "jMMMM jDD"
        };
        break;
  ...
  const initalChart = () => {
    ...
    chart.colors.list = defaultTheme.colors; // export options

    if (exportable) {
      chart.exporting.menu = new am4core.ExportMenu();
      chart.exporting.filePrefix = exportFilePrefix ? exportFilePrefix : getCurrentDate(persianMode ? "persian" : "gregorian");
      chart.exporting.menu.items = exportMenuItems ? exportMenuItems : [{
        label: `<p>&#9776;</p>`,
        menu: [{
    ...
    chart.dateFormatter.utc = dateFormatterIsUtc; // change date aixis date

    dateAxis.renderer.labels.template.adapter.add("text", (value, target) => {
      const dateObject = target.dataItem.dates.date;
      let jalaliDate = null;
      if (dateObject) jalaliDate = momentJalali(dateObject).format(persianMode ? timeRangeValue.jalaliFormat : timeRangeValue.format);
      return jalaliDate;
    ...
    if (timeRangeValue.format.length) dateAxis.dateFormats.setKey(timeUnit, timeRangeValue.format);
    dateAxis.min = timeRangeValue.min;
    dateAxis.max = timeRangeValue.max;
    dateAxis.renderer.grid.template.stroke = am4core.color(defaultTheme.gridColor);
    dateAxis.renderer.grid.template.strokeOpacity = defaultTheme.gridOpacity; // style category axis labels

    dateAxis.renderer.labels.template.fill = am4core.color(defaultTheme.labelsColor);
    dateAxis.tooltipDateFormat = timeRangeValue.format;
    const valueAxis = chart.yAxes.push(new am4charts.ValueAxis()); //style value axis grid

    valueAxis.renderer.grid.template.stroke = am4core.color(defaultTheme.gridColor);
    ...
    series.tooltip.background.fill = am4core.color(defaultTheme.seriesTooltipBackgroundColor);
    series.tooltip.background.opacity = defaultTheme.seriesTooltipBackgroundOpacity;
    ...
    series.tooltipHTML = seriesTooltipHtml ? seriesTooltipHtml : defaultSeriesTooltipHtml;
  ...
  return /*#__PURE__*/React.createElement("div", {
    id: id,
    className: `chart-root ${className}`
  });
};
